...
The review found and fixed four authority/data-consistency defects with additive migrations. No hosted database was changed.
...
All 23 migrations applied from scratch on isolated PostgreSQL 18 with minimal local substitutes for Supabase auth/storage schemas. These substitutes exercise PostgreSQL policies, roles, functions, and transactions, but do not test hosted PostgREST or real JWT handling.
...
- Animal ownership/profile policies still allow direct animal selection/ownership mutation outside the intended unlock RPC. This is a remaining authorization issue for a follow-up migration.
- Legacy exchange request policies still permit participant status transitions without recipient consent. The V2 UI no longer exposes that legacy workflow, but its database endpoint remains.
