...
## 7) Data Structure Choices (Why + Alternative + Trade-off)

| Area | Chosen data structure | Why chosen | Alternative(s) | Trade-off |
|---|---|---|---|---|
| Wallet balance | `user_wallets` keyed by `user_id` | O(1) balance read, simple mutation path | Event ledger only (sum at read), materialized view | Need strict write consistency and race handling |
| Official inventory | `user_inventory(user_id, item_id) -> quantity` (composite PK) | Compact quantity model, easy increment/decrement | One row per owned item instance | Less per-instance metadata flexibility |
| Room placements | `room_placements(room_id, anchor_id)` unique + `item_id` | Deterministic anchor rendering and simple collision control | Free XY coordinates + overlap engine | Lower placement freedom |
| Custom gallery placements | `custom_gallery_placements(user_id, listing_id)` + unique cell `(user_id, cell_x, cell_y)` | Guarantees one collectible one location and one item per cell | JSON blob layout per user profile | Harder to query/validate in JSON, weaker constraints |
| Claim dedup | `listing_claims` unique `(user_id, listing_id)` | DB-level idempotency | Client-side “already claimed” check only | Requires migration discipline and proper unique index maintenance |
| Author lookup in feed | In-memory `Map<userId, profile>` after batch fetch | Reduces repeated profile scans in render mapping | N+1 profile queries per card | Slightly more merge logic client-side |
| Room anchors | Constant arrays in `roomLayout.ts` with typed anchor objects | Fast deterministic UI and easy per-room-type variants | DB-driven dynamic anchors | Less runtime configurability; requires release for layout changes |
| ASCII companion frames | Constant dictionary object `species -> activity -> [frames]` | Tiny payload, cross-platform deterministic animation | Sprite sheet / video / lottie assets | Simpler visuals, less art richness |
