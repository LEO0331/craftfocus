...
export default function HomeScreen() {
  ...
  const loadDashboard = useCallback(async () => {
    ...
    try {
      ...
      setRecentActivity(posts.slice(0, 3).map((post) => t('home.activityLine', { author: post.author_name, title: post.title, category: post.category })));
    } finally {
      setIsLoading(false);
    }
  }, [user?.id]);
  ...
  return (
    ...
    >
      ...
      <View style={styles.metricsRow}>
        ...
        <Card style={styles.metricCard}>
          <Text style={styles.title}>{t('home.roomItems')}</Text>
      ...
      <Card>
        ...
        {!recentActivity.length ? <Text style={styles.hint}>{t('home.noActivity')}</Text> : null}
